...
def get_access_token() -> str:
    ...
    # Microsoft may return a replacement refresh token.
    # Always retain the newest one locally.
    ...
    # Always use the newest token for subsequent refreshes
    # during the current process.
        os.environ["MICROSOFT_REFRESH_TOKEN"] = new_refresh_token

    # Persist locally only when a local .env already exists.
    # GitHub Actions will provide credentials as environment
    # variables and should not create a credential file.
        env_path = Path(".env")
...
def ensure_receipts_folder() -> dict:
    ...
    if response.status_code == 200:
        ...
        print("OneDrive folder already exists: /Receipts")
        return folder
...
def upload_to_receipts(
    ...
    Intended for files small enough for Microsoft Graph's simple
    upload endpoint.
    """
    access_token = get_access_token()
